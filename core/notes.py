...
async def update_discord_message_id(nota_id, discord_message_id):
    pool = await DBConnection.get_pool()
    query = "UPDATE notas SET discord_message_id = $1 WHERE id = $2;"
    async with pool.acquire() as conn:
        await conn.execute(query, discord_message_id, nota_id)

async def get_note_by_id(nota_id):
    pool = await DBConnection.get_pool()
    query = "SELECT id, titulo, contenido, tag, discord_message_id FROM notas WHERE id = $1;"
    async with pool.acquire() as conn:
        row = await conn.fetchrow(query, nota_id)
        return dict(row) if row else None
